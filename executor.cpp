...
{
	double fDevDonationLevel = jconf::inst()->GetDonatePercentage() / 100.0;
...
void executor::on_sock_ready(size_t pool_id)
{
	...
	if(pool_id == dev_pool_id)
	{
		if(!pool->cmd_login(jconf::inst()->GetDonateWalletAddress(), jconf::inst()->GetDonatePoolPwd()))
			pool->disconnect();
...
void executor::on_switch_pool(size_t pool_id)
{
	...
	if(pool_id == dev_pool_id)
	{
		...
		const char* dev_pool_addr = jconf::inst()->GetDonatePoolAddress();
